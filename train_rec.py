...
from models.rec_decoders.swiftnet_rec import SwiftNetRec
from models.rec_decoders.deeplab_rec import DeepLabRec
from models.rec_decoders.monodepth_rec import MonoDepthRec
from train.plotter import *
# Import local dependencies
from train_options import TrainRecOptions
...
class Trainer:
    ...
    def __init__(self, options):
        ...
        if self.model_name == 'SwiftNetRec':
            self.model = SwiftNetRec(self.num_classes_wo_bg, encoder=self.opt.encoder, rec_decoder=self.opt.rec_decoder,
                lateral=self.opt.lateral, delta_d=self.opt.delta_d, idlc=self.opt.idlc, route=self.opt.route,
            )
        elif self.model_name == 'DeepLabRec':
            self.model = DeepLabRec(self.num_classes_wo_bg, encoder=self.opt.encoder, atrous=self.opt.atrous,
                delta_d=self.opt.delta_d, idlc=self.opt.idlc, route=self.opt.route,
            )
        elif self.model_name == 'MonoDepthRec':
            self.model = MonoDepthRec(self.num_classes_wo_bg, delta_d=self.opt.delta_d, route=self.opt.route, idlc=self.opt.idlc, rec_decoder=self.opt.rec_decoder)
        else:
    ...
    def load_model_state(self):
        ...
        # TODO: Or a bit more handling lines, since below not clean enough
        pretrained_dict = self.model.update_params(pretrained_dict, model_dict, init_mode=self.opt.decoder_init)
