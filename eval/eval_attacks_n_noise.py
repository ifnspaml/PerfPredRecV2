    ...
    def __init__(self, options):
        ...
        if self.model_name == "SwiftNet":
            ...
            self.model = load_model_def(self.model_name, self.num_classes_wo_bg, rec_decoder=self.opt.rec_decoder,
                                        backbone=self.opt.encoder, lateral=self.opt.lateral, idlc=self.opt.idlc)
        elif self.model_name == 'DeepLabRec':
            self.model = load_model_def(self.model_name, self.num_classes_wo_bg, backbone=self.opt.encoder, idlc=self.opt.idlc) 
        elif self.model_name == 'MonoDepthRec':
            self.model = load_model_def(self.model_name, self.num_classes_wo_bg, idlc=self.opt.idlc, rec_decoder=self.opt.rec_decoder) 
        else:
        ...
        if self.model_name in ['SwiftNetRec', 'DeepLabRec', 'MonoDepthRec']:
