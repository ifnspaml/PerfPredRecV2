...
MODES = ["segmentation", "reconstruction"]

class LossPlotter:

    def __init__(self, path='loss_plot.pdf', lr_scale=1e6, loss_scale=1e2, max_epoch=200, mode="segmentation"):
        super().__init__()
        self.path = path
        # For visualization
    ...
    def append_data(self, **kwargs):
        if self.mode == 'segmentation':
            self.append_data_seg(**kwargs)
        elif self.mode == 'reconstruction':
            self.append_data_rec(**kwargs)

    def save_loss_fig(self):
        if self.mode == 'segmentation':
            self.save_loss_fig_segmentation()
        elif self.mode == 'reconstruction':
            self.save_loss_fig_rec_standard()

    def append_data_seg(self, epoch_, train_loss, current_lr, val_loss, mIoU):
    ...
    def append_data_rec(self, epoch_, train_loss, current_lr, val_loss, psnr):
        self.x.append(epoch_)
        self.y_train_loss.append(train_loss * self.loss_scale)
        self.y_learning_rate.append(current_lr * self.lr_scale)
        self.y_val_loss.append(val_loss * self.loss_scale)
    ...
    def save_loss_fig_rec_standard(self):
    ...
    def save_loss_fig_segmentation(self):
        fig = plt.figure(figsize=(3, 3), dpi=300)
